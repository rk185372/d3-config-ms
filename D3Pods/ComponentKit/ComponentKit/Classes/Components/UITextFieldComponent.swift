...
open class UITextFieldComponent: UITextField, Stylable, NibInjectable {
    ...
    public func configureComponent(withStyle styleKey: TextFieldStyleKey? = nil) {
        var newStyle: TextFieldStyleKey

        if let styleKey = styleKey {
            newStyle = styleKey
            self.style = styleKey.rawValue
        } else {
            newStyle = TextFieldStyleKey(rawValue: style!)!
        }
        ...
        let componentStyle: AnyComponentStyle = componentStyleProvider[newStyle.rawValue]
